...
| `TestAgent` | test | `workspace/tests/`、`tests.md` | 写测试，只能请求固定 sandbox check |
| `ReviewAgent` | review | `review.md` | 受限只读项目和 runtime 摘要 |
...
CrewAI 执行异常由 GraphRunner 转为失败节点。当前测试失败是否触发修复，尚未成为 Agent/Workflow 的结构化协议。
