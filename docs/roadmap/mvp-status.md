...
| 模块 | 已实现 | 当前限制 | 下一步 |
|---|---|---|---|
| Project | 项目根目录、`workspace/`、`project.yaml`、默认 `runtime.yaml` | 无项目生命周期状态持久化 | 引入项目运行记录 |
| Planner v0 | 受控上下文、JSON 草案、一次修复、DAG 与交付前置校验 | 每 Agent 每计划一次；无运行反馈重规划 | `WorkItem` 与 PlanPatch |
| Workflow | `ExecutionPlan`、同步 GraphRunner、节点状态、能力缺口暂停 | 不持久化、不可恢复、无并发/重试 | 状态机与恢复接口 |
...
1. Planner 以结构化 WorkItem 表达任务、依赖、状态和完成条件。
